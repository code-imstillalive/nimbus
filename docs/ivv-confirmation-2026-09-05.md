...
## Cards deployed on the live install (6 Sep, 10:40 AEST)

Deployed the two v0.94.124 cards via ha-mcp's dashboard tools: new storage dashboard **Nimbus** (`/nimbus-cards`) with a Control Panel view (`custom:nimbus-dispatch-card-v4`) and a Regret view (`custom:nimbus-regret-card` for 1 and 2 days ago). Created the two helpers the Control Panel needs, `input_select.nimbus_dispatch_mode` (Automatic) and `input_boolean.nimbus_live_dispatch_armed` (off, nothing reads it yet), plus a template sensor `sensor.nimbus_card_battery_power` that negates the SigEnergy battery power because the card assumes positive = discharging while this install's sensor and Solver flag are positive = charging (filed upstream). All three card files confirmed served over the remote URL with HTTP 200 and `text/javascript`; no new log entries. Rendering not screenshot-verified (the ha-mcp screenshot engine is not installed here).
