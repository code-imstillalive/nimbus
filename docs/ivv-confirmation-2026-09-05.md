...
| #350 LTS forward-fill duplicates training rows | H | v0.94.75 | ☑️ ⚠️ | Code present. **Cannot be validated live: the LTS/hybrid path crashes before it gets here (S1 below).** |
...
| #353 NaN poisons the model | M | v0.94.70, v0.94.93 | ☑️ ⚠️ | `isfinite` guard and staleness cap present; not reachable live until S1 is fixed. |
...
Until S1 ships, an install with `training_source: hybrid` or `lts` will not train. Switching the hub option to `recorder` and calling `nimbus_load.retrain` restores forecasts within a few minutes (recorder retention permitting). Not applied here — left for the operator.
