...
def _clear_load_forecast_error_notification_if_needed() -> None:
    ...
    except Exception:  # noqa: BLE001, S110 -- a failed dismiss/sentinel-file removal must never break the solve; nothing to log or react to beyond that
        pass
...
def resolve_price_spike_override(
    ...
    - `effective_override_kw` is the value that actually gets passed
      ...
      household's own explicit scope decision: P2P's "consistency of
      delivery is itself part of what earns the rate" reasoning takes
      priority over a spike response). Human stays in the loop for the
      actual discharge decision -- arming is a deliberate, explicit
      action, not inferred from the threshold/alert alone.

    threshold<=0 (the default) means "no threshold configured" -- never
    fires on its own. The alert entity is optional and read
    defensively (unavailable/missing/fetch-failure all fail open to
    "no alert").
    """
...
def main() -> None:
    ...
    _wait_s = seconds_to_settlement_capture(datetime.now(UTC).astimezone(BRISBANE_TZ))
    if _wait_s > 0:
        print(
            f"[{datetime.now(UTC).astimezone(BRISBANE_TZ).isoformat()}] "
    ...
    if load_forecast_entities:
        ...
        # the outer sum -- so the decision to refuse lives here.
        #
        # This path has no RuntimeError-retry loop the way the
        # integration does; a cron run that cannot trust its load
        # forecast should publish nothing and let the next minute's tick
        # try again, which is what returning early does.
        summed_error = near_zero_summed_load_error(
            load_kw,
            failed_load_entities,
            _cfg_num(cfg, "solver_inverter_self_consumption_kw", 0.0),
        )
        if summed_error is not None:
            print(
                f"[{now.isoformat()}] Nimbus Solver: {summed_error}",
                file=sys.stderr,
            )
            return
    else:
    ...
    ha_post_state(
        ...
        round(load_kw[0], 3),
        {
            ...
            # this block's own comment further up).
    ...
    # the full mechanism. spike_detected is pushed below regardless of
    # whether spike_override_kw ends up armed/active.
    ...
    # dispatch): now reads live from cfg (number.nimbus_solver_
    # intraplan_smoothness_weight_kw, dashboard-editable) instead of
    # always silently passing network.py's own DEFAULT_SMOOTHNESS_
    # WEIGHT_KW constant -- falls back to that same constant, so an
    # already-configured household sees zero behaviour change until they
    # actually tune it up.
    ...
    ha_post_state(
        ...
        round(float(net_battery[0]), 3),
        {
            ...
            # nimbus issue #429: real live meter reading, distinct from
            # the forecast-vs-forecast fields above -- see this block's
            # earlier comment for the full reasoning.
            ...
            # DETECTED, independent of whether the override is armed.
