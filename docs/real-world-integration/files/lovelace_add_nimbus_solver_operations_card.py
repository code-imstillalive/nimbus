...
def main():
    ...
    for path in CANDIDATE_FILES:
        ...
        if view is not None:
            ...
            if section is not None:
                ...
                print(f"  {r1} operations card, {r2} counterfactual card ({len(section_cards)} cards total).")
            else:
                ...
                print(f"  {r1} operations card, {r2} counterfactual card ({len(view_cards)} cards total).")
            break
    ...
    if target_path is None:
        ...
        view = views[0]
        if "sections" in view:
            view["sections"].append({"type": "grid", "cards": [OPERATIONS_CARD, COUNTERFACTUAL_CARD]})
