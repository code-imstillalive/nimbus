...
def publish_offer_curve(plan) -> None:
    ...
    lower of the two colliding prices) and logs a warning rather than
...
def _build_offer_curve_dicts(
    ...
    for i, (price, kw) in enumerate(curve):
        ...
        if key in curve_dict:
            _LOGGER.warning(
                "Nimbus Solver: offer curve (%s) had two sweep points round to "
                "the same %s price key -- keeping the first (lower-price) one, "
                "dropping the later duplicate. Real values: kept=%.6f, "
                "dropped=%.6f",
                direction,
                key,
                curve_dict[key],
