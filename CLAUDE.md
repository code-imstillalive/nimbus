...
**Version: v0.94.97** (up from v0.94.81 at the start of 2026-09-04). A long, continuous run through Mark Purcell's codebase-review parent issue (#336) across 2026-09-04 into 2026-09-05: #355, #357 (partial, see below), #368, #349, #351, #354, #353, #356 (all 4 items), and #348 (all 4 findings) are now CLOSED. #360 (test infrastructure) is a substantial partial fix (findings 1/2/3/4/6 done, finding 5 investigated/partially left open, finding 7 — ml/ coverage gaps — not yet started) — see its own GitHub comments for the itemized breakdown rather than re-deriving it here. #357 itself stays open, not because more code work is needed, but because a precise, itemized 25+1 real writer-script content-difference list was handed to Mark for his own judgment call on whether it needs a stricter enforcement mechanism — don't re-litigate that list yourself if you pick this up, read the issue's own comments for where it was left. Full issue-by-issue detail (exact fixes, mutation-testing evidence, real numbers) lives in each release's own `CHANGELOG.md` entry (v0.94.89 through v0.94.97) — this section intentionally doesn't re-summarize all of it.
