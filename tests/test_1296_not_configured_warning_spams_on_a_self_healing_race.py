...
def _reset_module_state() -> None:
    ...
    # The counter the fix introduced. Without this the second test in a run
    # inherits the first one's count and crosses the warn threshold on its
    # own first occurrence -- the tiering is per-consecutive-run, so the
    # harness has to start from zero the same way a fresh install does.
