...
class TestPriceRiskAversion(unittest.TestCase):
    def test_zero_risk_aversion_is_a_complete_noop(self):
        ...
        plan_a = build_plan(periods=periods, grid=grid_a, battery=battery, solar=solar, loads=loads,
                             import_price_risk_aversion=0.0, export_price_risk_aversion=0.0)
        plan_b = build_plan(periods=periods_b, grid=grid_b, battery=battery, solar=solar, loads=loads,
                             import_price_risk_aversion=0.0, export_price_risk_aversion=0.0)
    ...
    def test_none_bounds_is_a_complete_noop_even_with_nonzero_risk_aversion(self):
        ...
        periods_b, grid_b, _, _, _ = _scenario()
        plan_a = build_plan(periods=periods, grid=grid, battery=battery, solar=solar, loads=loads,
                             import_price_risk_aversion=1.0, export_price_risk_aversion=1.0)
        plan_b = build_plan(periods=periods_b, grid=grid_b, battery=battery, solar=solar, loads=loads,
                             import_price_risk_aversion=0.0, export_price_risk_aversion=0.0)
        np.testing.assert_allclose(plan_a.grid_import_kw, plan_b.grid_import_kw)
        np.testing.assert_allclose(plan_a.grid_export_kw, plan_b.grid_export_kw)
    ...
    def test_full_risk_aversion_uses_the_full_upper_bound_for_import(self):
        ...
        periods0, grid0, battery, solar, loads = _scenario(import_price_upper=np.array([0.80] * 10))
        periods1, grid1, _, _, _ = _scenario(import_price_upper=np.array([0.80] * 10))
        plan0 = build_plan(periods=periods0, grid=grid0, battery=battery, solar=solar, loads=loads,
                            import_price_risk_aversion=0.0, export_price_risk_aversion=0.0)
        plan1 = build_plan(periods=periods1, grid=grid1, battery=battery, solar=solar, loads=loads,
                            import_price_risk_aversion=1.0, export_price_risk_aversion=0.0)
    ...
    def test_export_price_lower_is_symmetric_for_discharge(self):
        ...
        periods0, grid0, battery, solar, loads = _scenario(export_price_lower=np.array([0.05] * 10))
        periods1, grid1, _, _, _ = _scenario(export_price_lower=np.array([0.05] * 10))
        plan0 = build_plan(periods=periods0, grid=grid0, battery=battery, solar=solar, loads=loads,
                            import_price_risk_aversion=0.0, export_price_risk_aversion=0.0)
        plan1 = build_plan(periods=periods1, grid=grid1, battery=battery, solar=solar, loads=loads,
                            import_price_risk_aversion=0.0, export_price_risk_aversion=1.0)
    ...
    def test_import_and_export_dials_are_fully_independent(self):
        ...
        for import_risk in (0.0, 1.0):
            periods_with, grid_with, battery, solar, loads = _scenario(import_price_upper=up, export_price_lower=low)
            periods_without, grid_without, _, _, _ = _scenario(import_price_upper=up, export_price_lower=None)
            plan_with = build_plan(periods=periods_with, grid=grid_with, battery=battery, solar=solar, loads=loads,
                                    import_price_risk_aversion=import_risk, export_price_risk_aversion=0.0)
            plan_without = build_plan(periods=periods_without, grid=grid_without, battery=battery, solar=solar, loads=loads,
                                       import_price_risk_aversion=import_risk, export_price_risk_aversion=0.0)
            np.testing.assert_allclose(plan_with.grid_import_kw, plan_without.grid_import_kw,
                                        err_msg=f"export bound leaked into the plan with import_risk={import_risk}, export_risk=0.0")
            np.testing.assert_allclose(plan_with.grid_export_kw, plan_without.grid_export_kw,
                                        err_msg=f"export bound leaked into the plan with import_risk={import_risk}, export_risk=0.0")
        ...
        for export_risk in (0.0, 1.0):
            periods_with, grid_with, battery, solar, loads = _scenario(import_price_upper=up, export_price_lower=low)
            periods_without, grid_without, _, _, _ = _scenario(import_price_upper=None, export_price_lower=low)
            plan_with = build_plan(periods=periods_with, grid=grid_with, battery=battery, solar=solar, loads=loads,
                                    import_price_risk_aversion=0.0, export_price_risk_aversion=export_risk)
            plan_without = build_plan(periods=periods_without, grid=grid_without, battery=battery, solar=solar, loads=loads,
                                       import_price_risk_aversion=0.0, export_price_risk_aversion=export_risk)
            np.testing.assert_allclose(plan_with.grid_import_kw, plan_without.grid_import_kw,
        ...
        periods_imp_on, grid_imp_on, battery, solar, loads = _scenario(import_price_upper=up)
        periods_imp_off, grid_imp_off, _, _, _ = _scenario(import_price_upper=up)
        plan_imp_on = build_plan(periods=periods_imp_on, grid=grid_imp_on, battery=battery, solar=solar, loads=loads,
                                  import_price_risk_aversion=1.0, export_price_risk_aversion=0.0)
        plan_imp_off = build_plan(periods=periods_imp_off, grid=grid_imp_off, battery=battery, solar=solar, loads=loads,
                                   import_price_risk_aversion=0.0, export_price_risk_aversion=0.0)
        self.assertFalse(np.allclose(plan_imp_on.grid_import_kw, plan_imp_off.grid_import_kw),
