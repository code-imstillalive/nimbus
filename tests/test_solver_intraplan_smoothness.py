...
def _flat_price_scenario():
    ...
    """
    n = 12
    periods = PeriodGrid(hours=np.full(n, 5.0 / 60.0), start=datetime(2026, 8, 20, 13, 15))
    grid = GridConfig(
        import_price=np.full(n, 0.0202), export_price=np.full(n, -0.0053),
        import_limit_kw=44.0, export_limit_kw=44.0,
    )
    battery = BatteryConfig(
        capacity_kwh=122.2, initial_soc_kwh=112.2, min_soc_kwh=2.44, max_soc_kwh=122.2,
        max_charge_kw=40.0, max_discharge_kw=40.0,
        charge_efficiency=0.926, discharge_efficiency=0.926,
        charge_cost=0.005, discharge_cost=0.01, salvage_value=0.15,
    )
    solar = SolarConfig(forecast_kw=np.full(n, 7.0), lower_kw=np.full(n, 6.0), upper_kw=np.full(n, 8.0))
    loads = [LoadConfig(name="load", forecast_kw=np.full(n, 5.8), lower_kw=np.full(n, 5.0), upper_kw=np.full(n, 6.5))]
...
def _price_step_scenario():
    ...
    battery = BatteryConfig(
        capacity_kwh=122.2, initial_soc_kwh=90.0, min_soc_kwh=2.44, max_soc_kwh=122.2,
        max_charge_kw=40.0, max_discharge_kw=40.0,
        charge_efficiency=0.926, discharge_efficiency=0.926,
        charge_cost=0.005, discharge_cost=0.01, salvage_value=0.15,
    )
    solar = SolarConfig(forecast_kw=np.full(n, 3.0), lower_kw=np.full(n, 2.5), upper_kw=np.full(n, 3.5))
    loads = [LoadConfig(name="load", forecast_kw=np.full(n, 2.0), lower_kw=np.full(n, 1.5), upper_kw=np.full(n, 2.5))]
    return periods, grid, battery, solar, loads
...
class TestIntraplanSmoothness(unittest.TestCase):
    def test_flat_price_burst_is_reproducible_without_the_fix(self):
        ...
        self.assertGreater(jag, 20.0, "expected the real degenerate burst pattern to reproduce with the fix off")
    ...
    def test_smoothness_weight_eliminates_burst_at_zero_extra_cost(self):
        ...
        """
        periods, grid, battery, solar, loads = _flat_price_scenario()
        plan_off = build_plan(periods=periods, grid=grid, battery=battery, solar=solar, loads=loads,
                               smoothness_weight=0.0)
        plan_on = build_plan(periods=periods, grid=grid, battery=battery, solar=solar, loads=loads,
        ...
        self.assertLess(jag_on, 1.0, "smoothness_weight should reduce the burst to essentially flat")
        self.assertAlmostEqual(plan_on.total_cost, plan_off.total_cost, places=2,
    ...
    def test_genuine_price_step_is_not_smeared(self):
        ...
        self.assertGreater(step_off, 50.0, "sanity check: the unfixed scenario really does have a big real step")
        self.assertGreater(step_on, 50.0, "the fix must not smear a genuine, large, real transition")
