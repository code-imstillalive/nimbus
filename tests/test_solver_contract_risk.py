...
class TestSolverStabilityUnderWrongContractAssumption(unittest.TestCase):
    ...
    def test_wildly_optimistic_volume_cap_does_not_break_the_solve(self):
        ...
        self.assertEqual(
            plan.status,
            "optimal",
            "an over-optimistic volume cap must never make the LP infeasible or crash",
        )
        ...
        self.assertLess(
            total_claimed,
            30.0,
            "claimed bonus volume must stay within what's physically deliverable, regardless of an unrealistic cap",
        )
...
class TestScoringDetectsRealMatchShortfall(unittest.TestCase):
    ...
    def test_lower_real_match_produces_worse_score_than_full_match(self):
        ...
        report_full_match = self._plan_and_report(
            real_p2p_dollars_earned=full_match_dollars
        )

        # Real shortfall: LocalVolts only actually matched half of it.
        half_match_dollars = full_match_dollars * 0.5
        report_half_match = self._plan_and_report(
            real_p2p_dollars_earned=half_match_dollars
        )

        self.assertLess(
            report_full_match.j_ach,
            report_half_match.j_ach,
        ...
        self.assertGreater(
            report_half_match.epr.epr,
            -1e9,  # sane, finite, not NaN/inf
        )
        self.assertLess(
            report_half_match.epr.epr,
        ...
        self.assertFalse(
            np.isnan(report_half_match.epr.epr),
            "EPR must stay a real, finite number even under a real shortfall, never NaN",
        )
