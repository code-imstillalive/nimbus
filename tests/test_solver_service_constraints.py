...
import numpy as np
from solver.elements import AdequacyLoadConfig, BatteryConfig, GridConfig, PeriodGrid, SolarConfig
...
class TestAdequacyHoldsUnderPriceStress(unittest.TestCase):
    ...
    def _run_at_import_multiplier(self, multiplier: float):
        ...
        battery = _base_battery(initial_soc_kwh=2.0, max_discharge_kw=1.0)  # deliberately weak battery -- forces real grid import to meet the target under stress
        adequacy = [AdequacyLoadConfig(name="hws", max_power_kw=3.7, target_kwh=5.0, earliest_period=1, deadline_period=2)]
        return build_plan(periods=periods, grid=grid, battery=battery, solar=solar, loads=[], adequacy_loads=adequacy)
    ...
    def test_target_is_met_at_normal_price(self):
        ...
        self.assertGreaterEqual(delivered_kwh, 5.0 - 1e-6, "the real target must be met at normal price")
    ...
    def test_target_is_still_met_at_2x_5x_10x_price(self):
        for multiplier in (2.0, 5.0, 10.0):
            with self.subTest(multiplier=multiplier):
                ...
                self.assertEqual(plan.status, "optimal", f"a real, physically-achievable adequacy target must stay satisfiable at {multiplier}x price, not go infeasible just because it's now expensive")
                delivered_kwh = plan.adequacy_loads[0].delivered_by_deadline_kwh
                self.assertGreaterEqual(
                    delivered_kwh, 5.0 - 1e-6,
    ...
    def test_cost_genuinely_rises_with_price_even_though_the_target_still_gets_met(self):
        ...
        self.assertGreater(
            plan_10x.total_cost, plan_1x.total_cost,
...
class TestAdequacyGenuineInfeasibility(unittest.TestCase):
    ...
    def test_physically_impossible_target_is_honestly_infeasible(self):
        n = 3
        periods = _flat_grid(n)
        grid = GridConfig(import_price=np.full(n, 0.30), export_price=np.full(n, 0.05), import_limit_kw=20.0, export_limit_kw=20.0)
        solar = SolarConfig(forecast_kw=np.zeros(n))
        ...
        adequacy = [AdequacyLoadConfig(name="hws_impossible", max_power_kw=3.7, target_kwh=100.0, earliest_period=0, deadline_period=0)]
        plan = build_plan(periods=periods, grid=grid, battery=battery, solar=solar, loads=[], adequacy_loads=adequacy)
        self.assertEqual(plan.status, "infeasible", "a physically-impossible adequacy target must surface honestly as infeasible, not a silently-adjusted or partial outcome")
