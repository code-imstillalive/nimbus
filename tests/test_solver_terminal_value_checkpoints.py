...
def _terminal_curve(base_rate: float, min_soc_kwh: float, max_soc_kwh: float) -> list[tuple[float, float]]:
...
def _scenario(period_indices):
    ...
    grid = GridConfig(
        import_price=np.full(n, 0.30), export_price=np.full(n, 0.15),
        import_limit_kw=44.0, export_limit_kw=44.0,
    )
    min_soc_kwh, max_soc_kwh = 122.2 * 0.05, 122.2 * 1.0
    battery = BatteryConfig(
        capacity_kwh=122.2, initial_soc_kwh=max_soc_kwh,
        ...
        charge_cost=0.005, discharge_cost=0.01,
        salvage_value=0.15,
...
class TestTerminalValueCheckpoints(unittest.TestCase):
    def test_checkpoint_makes_soc_meaningfully_higher_at_the_boundary(self):
        without_periods, without_grid, without_battery, without_solar, without_loads = _scenario(period_indices=None)
        without_plan = build_plan(periods=without_periods, grid=without_grid, battery=without_battery, solar=without_solar, loads=without_loads)

        with_periods, with_grid, with_battery, with_solar, with_loads = _scenario(period_indices=[DAY_BOUNDARY_IDX, FINAL_IDX])
        with_plan = build_plan(periods=with_periods, grid=with_grid, battery=with_battery, solar=with_solar, loads=with_loads)
    ...
    def test_final_period_still_credited_alongside_the_new_checkpoint(self):
        periods, grid, battery, solar, loads = _scenario(period_indices=[DAY_BOUNDARY_IDX, FINAL_IDX])
        ...
        floor_plus_15pct = battery.min_soc_kwh + 0.15 * (battery.max_soc_kwh - battery.min_soc_kwh)
        self.assertGreater(
            soc_at_final, floor_plus_15pct,
    ...
    def test_none_is_byte_identical_to_explicit_final_period_only(self):
        ...
        auto_periods, auto_grid, auto_battery, auto_solar, auto_loads = _scenario(period_indices=None)
        auto_plan = build_plan(periods=auto_periods, grid=auto_grid, battery=auto_battery, solar=auto_solar, loads=auto_loads)

        explicit_periods, explicit_grid, explicit_battery, explicit_solar, explicit_loads = _scenario(period_indices=[FINAL_IDX])
        explicit_plan = build_plan(periods=explicit_periods, grid=explicit_grid, battery=explicit_battery, solar=explicit_solar, loads=explicit_loads)

        self.assertEqual(auto_plan.status, "optimal")
        self.assertEqual(explicit_plan.status, "optimal")
        np.testing.assert_allclose(auto_plan.battery_soc_kwh, explicit_plan.battery_soc_kwh, atol=1e-6)
        self.assertAlmostEqual(auto_plan.total_cost, explicit_plan.total_cost, places=4)


def _base_kwargs(**overrides):
    kwargs = dict(
        capacity_kwh=100.0, initial_soc_kwh=50.0,
        min_soc_kwh=5.0, max_soc_kwh=100.0,
