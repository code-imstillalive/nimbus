...
- Devhub validation: **run, and it produced a stronger result than this entry originally claimed — plus a correction to the claim itself.** The original line said devhub "has no P2P settlement sensor configured, so `_settlement_entry_exists()` returns False there by design." **That was wrong, and it was never checked before being written.** devhub sees `sensor.lv_v2_p2p_confirmed_history` with 52 rows through 2026-09-23, mirrored from production, so the gate returns True there for any settled date.

  What was actually verified on devhub at v0.94.416: HACS reports `installed_version == available_version == v0.94.416`; a full restart is clean with **zero** `nimbus_load` WARNING or ERROR lines in the following hour; `solve_now` executes; and the behavioural marker is present in the place the change lives — devhub's own `sensor.nimbus_solver_quality_report_2` carries `nimbus_version: 0.94.416` and its newest row reads `{"epr": 0.416, ..., "v": "0.94.416", "r": "s", "p": 1}`. The new flag is written, and written only on the provisional day. The canonical `sensor.nimbus_solver_quality_report` on the same box still reads `v: 0.94.413` with no `p`, which is the #972 mirror lesson holding and a real before/after contrast rather than a version string alone.

  **The unplanned finding is the valuable one.** devhub scored 21/22/23 Sep at **89.85 / 87.74 / 87.25%** where production published **51.9 / 38.5 / 36.4%** for the same three days — same code, same mirrored input sensors, differing only in *when* each install happened to score them, before or after settlement landed. That is a ~50-point gap from scoring time alone, observed across two independent installs with nothing re-run, which is a cleaner confirmation of the cause than the manual rescore that motivated the fix.

  **What is still not verified: the sweep firing.** devhub's only flagged row is the currently-published day, which the sweep deliberately skips (that is #1082's job), so there is nothing there for it to repair yet. It becomes observable once that row stops being `latest_date` and its settlement has landed — roughly a day out. Not claimed until seen.
...
- **Forecast error as a function of lead time is now measurable from ordinary recorder history** ([#937](https://github.com/code-imstillalive/nimbus/issues/937)). `sensor.nimbus_household_load_total_forecast` gains three recorded scalars — `load_forecast_plus_1h_kw`, `load_forecast_plus_6h_kw`, `load_forecast_plus_24h_kw` — each the value the current forecast assigns to that lead time. Compare the figure recorded at time *T* against the real load at *T + lead* to get the curve.
  ...
  Why it was not measurable before: the full `forecast` array is in `_unrecorded_attributes` — correctly, since that is what keeps the per-period series from tripping the recorder's 16 KB per-attribute cap and taking every other attribute down with it ([#625](https://github.com/code-imstillalive/nimbus/issues/625)/[#890](https://github.com/code-imstillalive/nimbus/issues/890)). So no historical forecast existed at *any* lead time, and #937 had only two points on the curve: one-step-ahead, where the forecaster beats persistence by ~50% on MAE, and day-ahead, where its dispatch value came out negative on 11 of 14 real scored days. The shape between those endpoints is what separates "the model is wrong" from "the recursive multi-step path degrades" — and the latter is a failure mode this project has already fixed seven separate bugs in.

  Three floats cost nothing against the attribute budget and need no new storage, which is the same reasoning the household accepted for #919: recover it from history rather than writing a per-cycle store. `None` beyond the published horizon rather than clamped to the last period — a forecast that does not reach 24 h ahead has no 24 h-ahead value, and clamping would publish "the far end of whatever we had" under a key claiming a specific lead time, corrupting the very curve this exists to measure. The grid is deliberately non-uniform (5-min first tier, coarser later, [#438](https://github.com/code-imstillalive/nimbus/issues/438)), so the target instant is located against `grid_times` rather than by dividing.

  **Known limitation, stated plainly**: this is on the native/HACS publish only. The standalone/cron writer builds this sensor's attributes inline (the already-recorded `INTENTIONAL_NATIVE_ONLY` drift category), so an install running that path will not produce these scalars until it either moves to the integration or that copy is ported. Not a dispatch change — no plan, price or commanded state moves because of it.

  **One deployment caveat, checked after this merged:** these scalars are recorded normally on an install executing current code, because `forecast` is excluded from the recorder via `_unrecorded_attributes` and the remaining payload measures ~1.7 KB against the 16 KB cap. They will *not* record on an install whose executing code is stale — there the un-excluded `forecast` pushes the row over the cap and the recorder drops every attribute, `unit_of_measurement` included. Diagnosing that is a one-line check: `solve_diagnostics` carries seven keys on current code, and fewer means something older wrote the state. See [#890](https://github.com/code-imstillalive/nimbus/issues/890), which is closed as not-a-bug for exactly this reason.
...
- **v0.94.324's `binding_constraint_now` fix did not work, and this is the second pass at it** ([#921](https://github.com/code-imstillalive/nimbus/issues/921)). Deploying v0.94.325 and looking at the same install showed the attribute still reading:
  ...
  **What made the first attempt wrong was the shape of the test, not the arithmetic.** A small synthetic LP returns a pinned variable at exactly its bound — checked directly while diagnosing this, `delta = 0.000e+00` — which is what made `1e-6` look safe. That does not generalise to one variable out of a ~12,000-column two-phase MIP. Now compares against a real tolerance (`_PIN_MATCH_TOLERANCE_KW = 0.01`, 10 W — far below anything a household could act on).

  **The live residual is bounded, not guessed at.** `publish_plan()` writes `round(float(plan.grid_export_kw[i]), 3)`, and that install published exactly `12.0`, so the raw solved value sits within **0.5 W** of the 12.0 kW commitment. Combined with `1e-6` kW (0.001 W) having failed, the real residual is somewhere between 0.001 W and 0.5 W — so 10 W clears it by more than twenty times, and the number is anchored to a measurement rather than to a feeling about solver noise.
...
### Fixed
- **Two real Forecaster fields were missing from `docs/configuration-reference.md`**, which opens by claiming *"If a field isn't listed here, it doesn't exist yet — every real field this integration has, across every version, is covered below."* Documentation only.

  Added: **Training data source** (`training_source`) — Recorder history, long-term statistics, or Hybrid, and why the choice matters when a low `purge_keep_days` leaves less history than the training window asks for — and **Hybrid mode: recent days from full-resolution history** (`hybrid_recent_days`).

  Ten of the twelve Forecaster fields were already documented. That file is otherwise careful and honest about its own method (*"generated by hand from the real source so it stays accurate"*); this is a two-field gap, not a rotten document.

- **`docs/TESTERS.md` still told testers the Solver has never had operational control on the reference household, and still dated the add-on removal to a version that has never shipped.** Two independently wrong claims in a 41-line file last touched 2026-09-04. Documentation only.

  - The reference-household row read *"the Solver itself has never had operational control here, deliberately"*. That install runs real dispatch — real battery, real grid, real exported money. [#749](https://github.com/code-imstillalive/nimbus/issues/749) corrected exactly this claim across the docs on 09-11, but it did so by searching for the phrase **"shadow mode"**, and this copy of the claim is worded without it. A sweep keyed to one phrasing does not find a synonym, which is worth remembering the next time a claim is retired repo-wide: grep the *assertion*, not the wording.
  - The bug-report checklist dated the `nimbus_solver_app` add-on's removal to **v1.0.0**. It was removed in **v0.94.85** ([#357](https://github.com/code-imstillalive/nimbus/issues/357)), as `CHANGELOG.md`'s own header line says; there is no v1.0.0. A tester comparing their install against that number could not have placed themselves either side of it.

  Both lines now carry a dated note saying what they used to say, since a tester who read the old text is the person most likely to reread this file.

### Notes
- **A guard for that completeness claim was attempted and deliberately not shipped.** The claim is falsifiable, which normally makes it a good test candidate — but three versions of the check were each wrong in a different way, and the third showed why the approach cannot work:

  - Matching **config keys** against the doc reports **45 of 46 fields undocumented**. The doc is a human-readable table, so `temperature_sensor` appears nowhere; the label "Temperature sensor" does.
...
Devhub validation: deployed and restarted, `installed_version == available_version == v0.94.321`, solve `optimal` in 1.15 s, `nimbus_status` "Working well", `state` and `whole_house_live_now_kw` both **2.37**, no new errors. A guide-only change is not something a solve can verify — the deploy confirms it does not break a running install, and the two added menu entries were each checked against `strings.json`'s own `initiate_flow` menu labels and the real subentry type list before being written.
...
- **`solve_diagnostics.n_controllable_loads` was under-reporting, and it was misleading an open investigation** (nimbus issue [#773](https://github.com/code-imstillalive/nimbus/issues/773)). The field summed `sheddable_loads + adequacy_loads` only, silently omitting `thermal_loads` — a first-class controllable-load kind since [#774](https://github.com/code-imstillalive/nimbus/issues/774)/[#800](https://github.com/code-imstillalive/nimbus/issues/800), and the kind the one real thermal load in existence actually uses.
  ...
  **Provenance, corrected:** the original entry said this was "found live" — six controllable loads visibly scheduled while the field reported `0`. That inference does not hold. The install it was observed on has a known, separately-tracked defect where the API-serving process and the solve-loop process are not the same process, with the solve loop running against a stale config entry carrying no controllable_load subentries at all — which predicts a `0` count on its own, and equally explains the `n_batteries: 1` seen alongside it despite a configured battery participant. The two observations may simply come from two processes. The defect below is real and verifiable by reading the source alone (`Plan` carries `thermal_loads`; the published expression did not reference it), so the fix stands — but it was found by reading code, not proven by an install.
...
- **A battery participant's power history is now scaled per row by its own recorded unit** (nimbus issue [#843](https://github.com/code-imstillalive/nimbus/issues/843), option A of Mark Purcell's own A/B/C steer — completing the Option C he chose; option B shipped in v0.94.287). `_kw_scale_factor()` reads a sensor's *current live* `unit_of_measurement` exactly once and applies that single scale across a whole day of fetched history, so it is structurally unable to see a sensor whose unit changes mid-window. Mark pulled the raw history with attributes and found the exact rows: his EV pack sensor reports `W` for ~80 seconds as the car wakes from sleep, then `kW`, and a live check made hours later returned `kW` — so `1514.417` was scaled by 1.0 and the quality report published ~1,500 kW of achieved battery power against a real ~80 kW fleet ceiling.
  ...
  **Deliberately a separate function with exactly one caller.** Preserving attributes makes a recorder read materially heavier — a separate join, on a report that fetches a full day per sensor per participant — so every other history read keeps the cheap attribute-stripped path. That scoping is what made option A affordable at all; a blanket change would impose the cost on every install to fix a wake transient on one sensor family. `_kw_scale_factor()` is untouched and still correct for its seven other callers, none of which fetch per-row units.
  ...
  13 new tests driving the REST branch with Mark's four real rows verbatim, asserting `[1.514417, -0.774818, -0.774994, -0.42282]` — a result no single whole-window scale can produce. Devhub cannot validate the fix itself — its `Test EV` participant uses synthetic sensors with no wake transient, so it never reproduced the failure; the real check is Mark re-running `compute_quality_report` for 13 Sep on his own data. Closes #843.
...
**Devhub validation (nimbus issue #594):** confirmed live on devhub after a real deploy-connectivity outage cleared (see the outage note below). `installed_version`/`available_version` both read `v0.94.273`, `pending_update: false`. `nimbus_load.solve_now` returned `status: optimal` on a fresh solve, `total_cost: -45.9108` (correctly rounded per the #756 convention, not raw float precision). `sensor.nimbus_health_report` showed 0 `recent_errors` post-deploy — the only warnings present were pre-existing `#757 diag` lines (a separate, already-tracked issue, unrelated to this release). This release's own fallback path was not itself exercised live during verification (no #773 failure fired in this window) — verification confirms the deploy is clean and the solver behaves normally, not that the fallback branch specifically ran.

**Unrelated to this release, logged for the record:** devhub was in a genuine connectivity outage (WebSocket handshake timeouts, `ENTITY_NOT_FOUND` on known-good entities) spanning the v0.94.271→272 and v0.94.272→273 deploy windows — distinct from this project's own previously-documented "stale deploy" bug (that one reports success with stale code; this one was the API itself being unreachable). Recovered on its own; no code or infrastructure change made in response.
...
### Added
- **New `kind=thermal` Controllable Load: a hard-constrained LP guarantee for "must heat every day"** (nimbus issue [#774](https://github.com/code-imstillalive/nimbus/issues/774)). The existing `kind=deferrable` (`AdequacyLoadConfig`) shape reaches its target via a *soft*, priced shortfall — the Hot Water Heat Pump's own guarantee broke five separate times in ~36h ([#726](https://github.com/code-imstillalive/nimbus/issues/726)/[#741](https://github.com/code-imstillalive/nimbus/issues/741)/[#733](https://github.com/code-imstillalive/nimbus/issues/733)/[#769](https://github.com/code-imstillalive/nimbus/issues/769)/[#770](https://github.com/code-imstillalive/nimbus/issues/770)/[#782](https://github.com/code-imstillalive/nimbus/issues/782)) across a dozen loosely-coordinated soft mechanisms, none of which ever had authority to make the guarantee genuinely hard. `kind=thermal` models tank temperature as a real LP state variable (`ThermalLoadConfig`/`ThermalLoadPlan`) with a *hard* deadline constraint — the same proven `must_have_soc_by_period_index` mechanism `BatteryConfig` already uses for a departure deadline, applied to temperature instead of SoC. Reuses `thermal_forecast.py`'s own flat idle-decay model (already learned from real recorder history) rather than a new ambient-scaled model, so the LP's hard guarantee and the dashboard's own temperature forecast can never disagree. An optional soft comfort floor (tier 3 of the objective hierarchy: heat every day > early > cheap > reheat if it gets too cold) triggers a real, priced mid-day reheat rather than relying on the device's own passive eco-mode floor. If a configured target genuinely can't be reached, `build_plan()` retries once with the deadline relaxed to the same soft shortfall-price shape `AdequacyLoadConfig` already uses (the [#477](https://github.com/code-imstillalive/nimbus/issues/477) lesson, applied here) — an unreachable thermal target now costs real money and shows up honestly (`Plan.thermal_guarantee_relaxed`) instead of taking the whole multi-day plan infeasible.
- Wizard support for `kind=thermal` on the Controllable Load subentry: temperature sensor entity, max power, target temperature, earliest/deadline hour, optional comfort floor + cost, optional heating-rate/idle-decay overrides.

### Changed
- **11 of 12 Controllable Load sensors reclassified to `EntityCategory.DIAGNOSTIC`** (nimbus issue #774, Mark Purcell's own entity-design ask): only `commanded_state` and `Status` remain on the main device page as the two primary, undifferentiated answers — Next Start/End, Planned Duration/Energy/Today, Delivered Today, Target Today, Planned/Today Cost, Cost Avoided, and Temperature Forecast move under "diagnostic entities." Mirrors the Quality sub-device's own already-shipped "one headline primary, rest diagnostic" pattern.

Prior art check: EMHASS's `docs/thermal_model.md` (cited directly on #774 by the maintainer) models tank temperature as an LP state variable with a hard bound — this reuses that idea with the project's own flat idle-decay model rather than EMHASS's ambient-scaled `cooling_constant` (no ambient-forecast plumbing exists in this codebase yet, named explicitly as a v1 simplification). HAEO's own thermal/EV-element source was not independently re-checked this pass. Explicitly out of scope for v1: ambient-scaled `cooling_constant`, a stronger earliness tie-break ([#769](https://github.com/code-imstillalive/nimbus/issues/769)'s own separate question), multi-window thermal deadlines, and extending the async thermal-rate-learner trigger to a load that has never been `kind=deferrable` (uses a config override or module defaults until then).

Validated: full local pytest suite (author's own Python 3.11 sandbox: 1475 passed, 0 failures, ~64 files excluded by a known import-collection gap; independently re-verified on full collection, Python 3.12: **2141 passed, 13 pre-existing skips, 0 failures**, twice in a row for reproducibility), CI green on the real target Python 3.14. A recorder-attribute-exclusion gap (new `plan_temperature_forecast` field missing from `NimbusControllableLoadStateSensor._unrecorded_attributes`, the #625 bug class) was found by CI's own guard test and fixed same-PR. `ruff check`/`ruff format --check` both clean (0 new findings vs `origin/main`), mypy delta check against a fresh `origin/main` worktree (only the already-accepted try/except no-redef pattern grows, no new error class).

**Devhub validation (nimbus issue #594):** downloaded via HACS, restarted, `installed_version == available_version == v0.94.270` confirmed. Fresh solve post-restart confirmed healthy (`status: optimal`, `total_cost` correctly rounded, `last_updated` advancing). HA log checked for new WARNING/ERROR lines — none attributable to this release (only the same pre-existing duplicate-unique-ID restart artifacts already tracked in `reference_devhub_nimbus_duplicate_entities`). No live `kind=thermal` fixture exists yet on devhub to exercise the new solver branch end-to-end — Mark's own HWS subentry migration from `kind=deferrable` to `kind=thermal` remains the real live-verification step, tracked as a follow-up per the PR's own honest scope note.

## [0.94.269] — 2026-09-13

### Added
- **A real per-day energy cap for deferrable Controllable Loads** (nimbus issue [#482](https://github.com/code-imstillalive/nimbus/issues/482), price-gated loads e.g. a Bitcoin miner) — new optional wizard field `deferrable_max_kwh_per_day` (`AdequacyLoadConfig.max_kwh_per_day`). A price-gated load (`value_per_kwh` configured, no real deadline pressure of its own) has nothing else bounding how much energy it draws once the switchboard's shadow price sits below its own credited value; this lets a household cap it at a real daily energy budget instead. Applied SEPARATELY PER REAL CALENDAR DAY (not once across the whole solve horizon) — same reasoning and the same `network.py` implementation pattern as `GridConfig`'s own existing `export_bonus_volume_kwh` two-tier cap: a single global cap on a multi-day horizon would let the LP greedily exhaust the whole allowance on the very first cheap day it sees. Falls back to one global cap across the whole horizon when the solve has no real calendar anchor (`PeriodGrid.start is None`), matching that same mechanism's own honest fallback. Independent of `windows` (#612) — a genuinely separate concept: windows say when a deadline-driven target resets, this caps total energy used per day regardless of window structure. 4 new tests (per-day cap forcing cross-day delivery on a 2-day horizon, global-horizon fallback, none-is-a-no-op, validation), full local suite green, `ruff check`/`format --check` clean, mypy delta +0 (the pre-existing tuple-shape mismatches in `regret.py`/`quality_report.py` are unrelated local-environment numpy-stub noise, unchanged by this PR — confirmed via a real before/after comparison). Still open from #482's own original scope: a dedicated entity-driven `value_per_kwh` (today it's a plain fixed number, not yet readable from a template sensor per-solve).

**Devhub validation (nimbus issue #594):** downloaded via HACS, restarted, `installed_version == available_version == v0.94.269` confirmed. Periodic solve cycle confirmed fresh and healthy post-restart (`status: optimal`, `last_updated` advancing); `total_cost` correctly reported at 4dp precision (v0.94.268's own rounding fix genuinely executing, not the usual devhub staleness lag). Zero new ERROR log lines in the hour following restart. No live devhub fixture exists yet for `max_kwh_per_day` itself (none of devhub's controllable-load test fixtures have `value_per_kwh`/`deferrable_max_kwh_per_day` configured) — verified via unit tests only, same honest gap as every other feature this session with no matching devhub fixture.

## [0.94.268] — 2026-09-13

### Changed
- **`sensor.nimbus_solver_quality_report`'s EPR/regret scoring now includes every configured EV `battery_participant`, not just the home battery** — jointly re-solving the oracle across the whole fleet and excluding real driving discharge from the home-grid balance (nimbus issue [#768](https://github.com/code-imstillalive/nimbus/issues/768), [#585](https://github.com/code-imstillalive/nimbus/issues/585), Mark Purcell — PR #795). Wires v0.94.266's `oracle_dispatch()`/`evaluate_realized_cost_multi()` mechanism into actual production use: `compute_quality_report()`/`compute_daily_quality_report()` reconstruct each configured `battery_participant`'s own real historical dispatch from its already-configured power/SoC sensors (`_resolve_battery_participant_history()`, new), gated to home-grid-relevant periods only via the same `battery_participant_available_entity` the live forward solve already uses for scheduling (`fetch_entity_state_history_range()`, new) — without this gate, a real trip's own propulsion discharge would be priced as if it exported into the household's grid balance, corrupting `j_ach`/EPR the same way the #299/#532 bugs once did. A single-battery install sees zero change. Deliberately not included: controllable/adequacy-load timing (no hourly delivered-energy history exists for a load the way a battery gives one for free), and multi-trip-day oracle gating (a real, separate, larger LP piece — Mark's own explicit scoping choice; a single round-trip day, the common case, is unaffected). 19 new tests (16 wiring + 3 gating), full local suite green (2093 passed), `ruff check`/`format --check` clean, one real `mypy` (solver+ml, strict) finding fixed same-PR (`sum()`'s implicit `int 0` default widened several fleet-aggregate arrays to `ndarray | Literal[0]`; fixed with an explicit `start=np.zeros(...)` on each).
- **Fixed a real, independently-confirmed CI flake** (rode along in the same PR after blocking its own first CI run): `total_cost` was the one published KPI field skipping the 4dp rounding every sibling field (`total_cost_with_fixed_costs`, `cost_breakdown`, `cost_band`) already applies, directly exposing HiGHS's own run-to-run floating-point non-determinism (~1e-13 magnitude) to `test_main_golden_output_guardrail.py` and any real downstream consumer. Rounded at the publish site in `solver_writer.py`; `compute_cost_breakdown()`'s own residual math is untouched (still uses the raw, unrounded value). Confirmed pre-existing on `main`'s own HEAD before this fix, not introduced by any specific release.

**Devhub validation (nimbus issue #594):** downloaded via HACS, restarted, `installed_version == available_version == v0.94.268` confirmed; `nimbus_load.solve_now` produced `status: optimal`; HA log checked for new WARNING/ERROR lines — none attributable to this release (same pre-existing #757/#773 residuals as recent prior releases). `sensor.nimbus_solver_quality_report`'s live shape did not yet show a `scored_participants` field beyond home-only, consistent with devhub's own long-documented, unrelated deploy-staleness issue (executing code lagging its own reported HACS version, 11th confirmed occurrence — see `project_devhub_deploy_not_taking_effect_2026_09_09` for the full history) rather than a problem with this release itself. Real confirmation of this release's own consumer-facing effect awaits Mark Purcell checking his own household install directly (tracked in #768).

## [0.94.267] — 2026-09-13

### Added
- **`climate.*` domain dispatch for Controllable Loads** (nimbus issue #756, follow-up to #534). Previously, a Controllable Load pointed at a `climate` entity (an AC zone, a heat pump in climate mode) could be scored and its Done condition evaluated, but Nimbus could never actually command it — `dispatch_commanded_state()` logged a warning and no-opped. Implemented via `climate.set_hvac_mode`: `"off"` unconditionally on an OFF transition (universally present in HA's own `HVACMode` enum, zero ambiguity), and a new, explicit, required-per-load `CONF_CONTROLLABLE_LOAD_CLIMATE_ON_HVAC_MODE` config field on an ON transition. **Deliberately not `climate.turn_on`/`turn_off`**: checked live against a real reference-household climate entity (`climate.aircon_b1`) — its own `supported_features` bitmask did not advertise the `TURN_ON`/`TURN_OFF` capability bits, so those services are not a safe universal substitute for real hardware. **Deliberately not a guessed single "on" mode either**: the same real entity's own `hvac_modes` included both `heat` and `cool`, so there is no safe universal default — per this project's own no-hardcoding standard, this is always an explicit household/installer choice, never inferred. A climate device with this field left blank logs a WARNING and dispatches nothing on ON transitions (never guesses); OFF transitions are unaffected. 4 new/updated regression tests (2 new climate-specific dispatch tests, 1 no-configured-mode no-op test, 1 pre-existing generic "unsupported domain" test repointed at a genuinely still-unsupported domain now that climate is implemented). Full local suite green, `ruff check`/`ruff format --check` both clean, mypy delta +0 real findings (the one +1 line is the same pre-existing dual-import false-positive class already present for every sibling const on that line). **Honest scope note**: implemented and verified via mocked-service unit tests only — devhub has zero safe pseudo-climate entities to dispatch-test against live (every `climate.*` entity there is a real mirror of the reference household's own production AC hardware, and this session deliberately never called a service against any of them). A real end-to-end live-dispatch verification needs a pseudo-climate MQTT fixture (matching the existing `mqtt_pseudo_water_heater.py` pattern) built with real filesystem access to devhub, which this session didn't have — flagged as a genuine remaining gap, not silently glossed over.

## [0.94.266] — 2026-09-13

### Changed
- **`oracle_dispatch()` (`solver/regret.py`) extended to jointly re-solve a whole multi-battery/multi-load fleet under perfect foresight, instead of a single battery/load** (nimbus issue #768 "Option 1", Mark Purcell — PR #791). Internal solver plumbing only: `backtest.py`'s `score_candidate_day()` (the one existing call site) is preserved byte-identical for its current single-battery/single-load use, and `compute_quality_report()` (the real scorer behind `sensor.nimbus_solver_quality_report`) does not call this function at all, so nothing on any live dashboard changes. This is the reusable mechanism a future full-fleet EPR/regret scorer needs — wiring it into production still needs a real historical EV/controllable-load delivery reconstruction (#768 itself flags this as not yet built) and is deliberately a separate, later piece of work. Verified: backward-compatible single-asset call produces a byte-identical `Plan`; a genuine two-battery joint solve allocates discharge to the cheaper battery rather than splitting evenly; a genuine joint battery+deadline-load solve schedules the load timing correctly (~$1.40 cheaper than a naive price-blind placement on the same real day) — the exact question #768 asks. 5 new tests plus 14 pre-existing, all passing; full suite, ruff, and mypy all clean against the same baseline as `main`. **Devhub validation (nimbus issue #594):** downloaded via HACS, restarted, `installed_version == available_version == v0.94.266` confirmed; `nimbus_load.solve_now` produced `status: optimal`; HA log checked for new WARNING/ERROR lines — none attributable to this release (same pre-existing #757/#773 residuals as v0.94.265). No consumer-facing check applicable: this release has zero dashboard/entity/wizard surface, per its own "Changed" note above.

## [0.94.265] — 2026-09-13

### Fixed
- **`terminal_value_credit` still ~$34 too large after v0.94.262's `soc_penalty` fix** (nimbus issue #788, Mark Purcell — real live finding, confirmed the excess-import release valve, nimbus issue #390, was genuinely active that day). Root cause: `grid_import_excess[t]`'s own LP cost is `(effective_import_price[t] + import_excess_penalty_rate) * hours[t]`, but `solver_writer.py`'s own published `net_cost` per period only ever priced the combined `grid_import_kw` (which already folds `grid_import_excess_kw` in) at the plain `effective_import_price` rate — the `import_excess_penalty_rate` markup itself had no explicit line item anywhere in `compute_cost_breakdown()`, so it fell entirely into the residual `terminal_value_credit`. Same misattribution class as #781's `soc_penalty` bug and #776's blended-objective bug before it — the third real production incident traced to this same "unnamed LP cost term silently absorbed into the residual" mechanism. Checked and ruled out the other two candidates #788 itself suggested: proximal/smoothness weights (tie-break-sized, ~$0.01/kWh, far too small to produce $34) and P2P export bonus revenue (already correctly folded into `grid_net` via `net_cost`). Fixed by adding `Plan.grid_import_excess_penalty_cost` — deliberately given a `0.0` default rather than made a required field, a direct lesson from v0.94.264/#785's own outage (a required `Plan` field addition missed one construction site and took the whole solver down) — computed directly from the same `import_excess_penalty_rate` local and solved `grid_import_excess` values the LP itself used, exposed as a new `grid_import_excess_penalty` key in `compute_cost_breakdown()`, subtracted from the residual. New real solver-level test proves the exact LP-matching dollar value on a genuine excess-import scenario; reconciliation tests added; golden-output fixture regenerated. Confirmed: `ruff check` + `ruff format --check` both clean, zero new mypy findings, full local suite 2069 passed / 13 skipped / 0 failed, real `hass_integration` CI harness green on PR #789 before merge. **Devhub validation (nimbus issue #594):** downloaded via HACS, restarted, `installed_version == available_version == v0.94.265` confirmed; `nimbus_load.solve_now` produced `status: optimal`; the new `grid_import_excess_penalty` key confirmed present in the live `cost_breakdown` attribute on the first post-restart solve; HA log checked for new WARNING/ERROR lines — none attributable to this release (the entries present are the already-tracked #757 duplicate-entity-ID residual and a #773 recurrence, both pre-existing and unrelated to this change).
...
### Fixed
- **`terminal_value_credit` was still 94x-1200x too large after v0.94.259, confirmed independent of `CalibratedOptions`** (nimbus issue #781, real household finding, filed within hours of that fix shipping — two live re-solves with `switch.nimbus_solver_calibrated_objective_enabled` confirmed OFF both times still showed `terminal_value_credit` of $1678.63 and $141.52 against an expected salvage credit of roughly $1.40-$1.50). Root cause: this was never a salvage-value bug at all — it was the soft min/max-SoC violation penalty (`soft_soc_penalty_per_kwh`, a deliberately large, "dominant by construction" $/kWh cost applied every period a battery sits below its configured floor, NOT scaled by `hours[t]`) firing correctly (real batteries genuinely sitting below their configured floor) but with zero explicit line item anywhere in the household-facing report. `solver_writer.py`'s `cost_breakdown()` only ever had 4 named terms (`grid_net`/`degradation`/`charge_fee`/`discharge_fee`); everything else in the LP's real primary cost — including this penalty — silently fell into the `terminal_value_credit` residual, misrepresenting a real "your battery is below its safety floor" warning as if it were salvage/terminal value earned. Fixed by exposing a new `Plan.soc_penalty_cost` field (`network.py`, computed directly from the same solved underfill/overfill variables and the same rate already used to cost them, so it can never drift from the LP's own real number) and a new explicit `soc_penalty` key in `cost_breakdown()`, correctly subtracted out of the residual. 3 new regression tests, including one that proves the real end-to-end LP computation (not just a hand-fed unit test) by forcing a real below-floor scenario and confirming `Plan.soc_penalty_cost` matches `rate × underfill_kwh` exactly; golden-output fixture regenerated (a `soc_penalty: 0.0` addition only — that fixture's own battery never dips below floor, so no other value changed). Full local suite green (2059 passed, 13 skipped); live devhub verification (checking the new `soc_penalty` key appears in the published `cost_breakdown`) tracked on #781.
...
### Fixed
- **Critical: `total_cost`/`terminal_value_credit` badly inflated on any solve using the calibrated-objective architecture** (nimbus issue #776, real production finding — a live 3-battery household solve reported `total_cost=$1335` with `terminal_value_credit=$1295` of that, ~700x larger than plausible). Root cause: `CalibratedOptions`' own final step (`_calibrate_blend_weight()` in `solver/lp.py`) runs one last blended solve so HiGHS's live basis reflects a genuine single solve at the calibrated weight — but that solve's own objective value is `primary + weight * secondary`, a mixed-unit number (real dollar primary cost plus whatever arbitrary scale the secondary/tie-break cost uses), since calibration only bounds how far the blended optimum's PRIMARY cost can drift from lex-optimal, never the secondary term's own absolute magnitude. `_solve_highs()` was reading this mixed value straight off `h.getObjectiveValue()` as `LPResult.objective`, which `network.py::build_plan()` assigns directly to `Plan.total_cost` — every downstream dollar figure (including `solver_writer.py`'s `cost_breakdown()`, which computes `terminal_value_credit` as a deliberate RESIDUAL against `total_cost`) inherited the contamination. `LexOptions` doesn't have this bug (its own phase 3 re-solves on primary-only cost before returning), which is why this was calibrated-objective-specific. Fixed by having `_calibrate_blend_weight()` also compute and return the real primary-only cost of its actual final solution, threaded through `_solve_with_options()`/`_solve_highs()` as an explicit override for `.objective` on the `CalibratedOptions` path only — `BlendedOptions`/`LexOptions`/`options=None` are byte-identical to before. One new targeted regression test (an extreme secondary/primary magnitude mismatch that reproduces the ~5-order-of-magnitude distortion directly) plus the pre-existing golden-output fixture regenerated (a small, expected ~0.0008 shift on that fixture's own small-scale secondary costs — nowhere near #776's real 700x, confirming the fix's magnitude scales with how badly primary/secondary units were actually mismatched). Full local suite green (2047 passed, 13 skipped); live devhub restart/solve verification (checking the real `cost_breakdown`/`terminal_value_credit` value on `sensor.nimbus_solver_battery_forecast`) tracked on #776.
...
- **One gap-targeted "backstop" solve per curve, only when a real gap remains.** After the walk and the mandatory retail solve both complete, if the walk resolved at least one real boundary and the retail solve's own HiGHS ranging doesn't already reach back to meet it, exactly one extra solve is run at the midpoint of that gap. Confirmed live, 2026-09-10, on this household's real data before this fix: export's own 6.41¢–9.21¢ band hid a genuine 5.48 kW → 11.37 kW jump that neither that cycle's walk nor its retail solve ever touched. Bounded cost (never more than one extra solve per curve); a gap narrower than 0.01¢/kWh is treated as already effectively covered, not worth a wasted solve. Confirmed on real data that no fixed manual sweep grid (several tried, log-spaced and otherwise) reliably out-performs the walk's own adaptive discovery — this backstop closes the one specific, real blind spot instead of attempting a broader redesign.
- **The Market Price Cap domain constant corrected from a $20.00 placeholder to the real, confirmed-current $23.20/kWh** (nimbus issue #705, resolving #675's own long-open "may already be a step behind the real current figure" question — confirmed directly by Mark Purcell). The Market Floor Price (−$1.00/kWh) is unchanged, already independently confirmed by #675.
- **`sensor.nimbus_offer_curve` gains a new `price_limits` attribute** — the real AEMO domain (`market_floor_price`, `market_price_cap`) the walk was bounded by this cycle, published as a genuine JSON object rather than only ever living as a Python constant with no way for a dashboard or household to read it.
- **`import_curve`/`export_curve` are now a list of self-contained band objects (`{"price_lower", "price_upper", "kw"}`), superseding v0.94.241's own `{price: kW}` dict shape** (nimbus issue #706). That dict shape's own key — a price rounded to 4 decimal places — turned out to be a real bug surface: two genuinely distinct real sweep points that happened to round to the identical displayed price silently collided, one kept and the other dropped, with only a log warning — confirmed live on this household's own data, six collisions in one solve cycle, one of them ~0.8 kW apart (not float noise). A list of bands has nothing to collide on; the separate `import_curve_ranging`/`export_curve_ranging` attributes are removed (each band now carries its own price range directly). Adjacent bands that are EXACT duplicates after rounding (same price range and kW) are still collapsed to one row — unlike the collision case, that loses no real information.
...
### Changed
- **The offer curve now walks its own real breakpoints instead of sampling a fixed 7-point grid** (nimbus issue #678, sub-issue of #674, Mark Purcell/PR #700). `_offer_curve_price_grid()` is removed; `_offer_curve_ranging_walk()` (`solver/network.py`) starts at the real AEMO Market Floor Price (−$1.00/kWh) and repeatedly calls `LPResult.sweep_cost_with_ranging()`, using each step's own exact `cost_up` boundary (nudged forward by a small relative epsilon) as the next price to solve at — finding every real segment between the floor and the Market Price Cap exactly, in as many re-solves as the curve actually has (typically 2-4 per side on real data, capped at 8), rather than hoping a fixed sample lands near a real breakpoint. Retail is always separately swept regardless, preserving the existing "curve at retail equals the main plan's own dispatch" guarantee. Confirmed live and in this project's own existing test fixture: the walk finds real structure the old fixed grid never sampled finely enough to see — a genuine 2.0 kW import plateau and a genuine 0.1 kW export plateau, both between −10¢ and 10¢, invisible to the old grid's coarser points. Supersedes #675's own "add the real forecast's own min/max as extra sweep points" ask (a walk that finds every breakpoint exactly needs no landmark points to guess where to look) — #675's separate, unconfirmed "is the $20 cap stale" question is untouched by this change.
- **Stage 2 of #696: the real production Solver now defaults to the new primary/secondary objective architecture (v0.94.239's `CalibratedOptions`) for its four existing tie-break mechanisms** (`proximal_weight`, `smoothness_weight`, `adequacy_earliness_budget_kw`, `battery_charge_earliness_budget_kw`) — household's own explicit, repeated instruction, not deferred. These mechanisms now resolve on the LP's SECONDARY objective channel with a real, per-solve-searched safe blend weight, replacing the previous hand-picked magnitudes summed directly into the primary cost. Verified this genuinely changes the real production golden-output scenario (not a no-op): `total_cost`, `total_cost_with_fixed_costs`, `terminal_value_credit`, and the binding-constraint/period shadow prices all shift by small, real, understood amounts — re-baselined in `test_main_golden_output_guardrail.py` after confirming the shift is the new architecture correctly engaging, not a regression.
- **New `switch.nimbus_solver_calibrated_objective_enabled`** ("Calibrated Objective Enabled") — a live, dashboard-editable escape hatch, matching this project's own established "every new solver mechanism gets a switch, not a silent hardcoded flip" convention. Default **ON**, unlike every other Solver switch in this project (all of which default off) — the one deliberate exception, since the household's own ask was to make this the real default now. A household can flip it off without a code deploy if the new architecture ever needs rolling back on their own live install.
- **One real, documented scope boundary, not a silent gap**: a solve that ends up a MIP (any `adequacy_loads` configured with `adequacy_semi_continuous=True`, the default) silently keeps today's hand-tuned-magnitude behavior regardless of this switch's state — `LPProblem`'s own primary/secondary architecture doesn't support a MIP yet (a genuinely new design question, not yet attempted, tracked in #696). Verified directly: a MIP scenario with the new switch enabled produces byte-identical output to the switch disabled, never raises, never silently diverges.
- Also wired into the "Nimbus Counterfactual" tracker's own separate `build_plan()` call, matching that function's own explicit design intent to never silently diverge from the real dispatch solve's own mechanisms.
- **Known, deliberate gap**: the legacy standalone-script fallback path (`docs/real-world-integration/files/nimbus_solver_forecast_writer.py`) was NOT updated to pass `solve_options` — it is not what devhub/NUC1's real production dispatch uses (see the README's own "Legacy standalone-script path" section), and re-syncing its own separate, ~4500-line copy of the production `build_plan()` call carries real risk of its own for a path few installs actually run. Left as a known follow-up, not silently ignored.
...
### Fixed
- **The battery's own charge decision had no preference for the earliest/cheapest tied moment, letting a critically-low battery sit idle through a perfectly reasonable price** (nimbus issue #692, household, real live plan mishaps directly investigated: identical import price — 7.62 c/kWh — produced 0 kW at 13:50 and −38.1 kW five minutes later at 13:55; separately, the real battery sat at 2-4% SoC for a full hour at a moderate 6-9 c/kWh price with nothing charging it). Root cause: #613's own earliness tie-break (prefer the earliest feasible period among genuinely tied options) was scoped to deferrable/sheddable loads only — the battery's own `charge_vars` never had an equivalent nudge, so the LP could arbitrarily defer charging as long as some later period still met its target. New `battery_charge_earliness_budget_kw` mechanism, mirroring #613's technique exactly but applied to every battery's own charge variable — a tiny, fixed total-budget-across-the-horizon cost gradient that never overrides a real price signal, only breaks a genuine tie toward charging sooner. On by default (magnitude matches `proximal_weight`/`smoothness_weight`, not #613's own larger load-side budget — verified live that the larger magnitude reintroduces the jagged charge/discharge burst those two mechanisms exist to eliminate, since a battery's own `max_soc_kwh` ceiling creates a real interaction a load's `target_kwh` delivery never has to contend with).
...
- **New live, dashboard-editable `number.nimbus_solver_battery_charge_earliness_budget_kw` ("Battery Charge Earliness Weight")** — same convention as `smoothness_weight`/`proximal_weight`, per a direct household ask to be able to tune this themselves rather than needing another release if the default ever proves insufficient. 0–0.1 $/kWh, default 0.005; values well above ~0.01 risk overriding genuine price signals and are not recommended.
...
### Fixed
- **A deferrable comfort load with `value_per_kwh` set would run at max power in every period below its price gate, delivering far more than its own real target** (nimbus issue #606, Mark Purcell — real finding on his own heat pump: surplus solar at 0¢/kWh feed-in from 07:30 went into the home battery instead, because the export-price *forecast* still showed 0.1-3.2¢ in the morning and the LP kept deferring to the "cheapest-on-paper" 11:00-13:30 block; the battery it filled instead wasn't needed until the 16:00 price step and reached 98% by 15:30 regardless). Root cause: `value_per_kwh`'s own credit was paid directly on raw per-period power with nothing capping the total credited energy at `target_kwh` — "the credit keeps the load on at max power in every cheap period and the published plan overstates energy and cost" (issue's own words), confirmed by an existing test that had (correctly, at the time) locked in exactly that behavior as intentional for #482. Fixed with a `served_credit[t]` variable, bounded both per-period (never more than what's actually delivered that period) and in aggregate (never more, cumulatively, than the load's own real target) — the load now fills the cheapest available periods first and stops once its target is met, verified empirically against the real HiGHS solver: a load needing both of two cheap periods now delivers `[5.0, 1.0, 0.0]` kWh instead of running full power through every period the price still gates in. Zero behavior change for the (currently universal) case where `value_per_kwh` isn't set. The wizard field's label and description were also rewritten in plain language ("run as soon as it's this cheap") — the underlying config field already existed and was already wired end-to-end, just described the old, overstated behavior and used "value credit" jargon instead of the household-facing framing this issue asks for.
- **Not this release**: pricing battery charging above a deferrable comfort load competing for the same surplus (#606's own proposal 2, EMHASS's `weight_battery_charge` / HAEO's Policy pricing) and modeling the tank as a real temperature state rather than a kWh quota (proposal 3, a new load kind, the same territory as #481) are both larger pieces, deliberately not attempted here — #606's own text frames proposal 1 (this fix) as the contained one and the other two as separate follow-ups.
...
### Added
- **Controllable Loads: each load's own full day-ahead plan is now published** (nimbus issue #581, Mark Purcell — real household ask: "chart the day-ahead plan for the heat pump," found impossible from outside the integration since the plan's own per-period power balance can't be honestly backed out of `sensor.nimbus_solver_battery_forecast`'s own aggregate rows). `sensor.nimbus_<load>_commanded_state` (the sensor #578/#579 already built) gains a `plan_forecast` attribute — the load's own `power_kw` (deferrable) / `served_kw` (sheddable) per period, in the same `{"time", "value"}` shape every other Nimbus forecast sensor already publishes — plus, for a deferrable load, `plan_delivered_kwh_forecast` (cumulative energy per period), `plan_target_kwh`, `plan_shortfall_kwh`, and the resolved `plan_earliest_period`/`plan_deadline_period` window; for a sheddable load, `plan_nominal_kw` (the flat forecast it was shed from). Refreshes every solve cycle regardless of whether `commanded_state` itself changed — a fresh schedule is the point — so, matching nimbus issue #362's own established reasoning for `NimbusHealthReportSensor`, all seven new attributes are excluded from long-term recorder history (they'd otherwise write a non-dedupable row every poll). The LP already computed this every cycle; `apply_commanded_state_guard()` previously read only period 0 of it and discarded the rest — this is purely additive publishing, no change to the existing commanded-state decision/dispatch logic.
...
### Fixed
- **A deferrable Controllable Load with a same-day window is no longer skipped for its entire active window** (nimbus issue #582, Mark Purcell — first live morning of the #534 heat pump, real repro at 06:01 with a 6am-4pm window). `_resolve_hour_to_period_index()` always resolves "the next occurrence of this hour from now," so once `now` passed today's earliest hour, earliest rolled forward to tomorrow while the still-ahead deadline stayed today — `build_controllable_loads()` then saw `deadline_period < earliest_period` and skipped the load every single cycle for the rest of the window, the opposite of the intended behavior (the window is exactly when deadline pressure should apply). Fixed exactly as diagnosed: when this ordering conflict is detected, the load's own unrolled earliest/deadline instants are checked directly — if `now` genuinely falls inside today's window, earliest_period resolves to "right now" (period 0) instead of tomorrow. The genuine overnight case (e.g. earliest 10pm, deadline 6am) is unaffected when checked ahead of when it opens. **Known remaining gap, not fixed by this pass**: an overnight window where `now` is already inside it (e.g. it's 11pm and the window opened at 10pm, or it's 3am and the window opened at 10pm the night before) still isn't detected by this same-day check and will still skip — a real, separate case from #582's own reported repro, left open for a follow-up issue.
...
### Fixed
- **`docs/real-world-integration/files/nimbus_solver_forecast_writer.py` (standalone/cron deployment) was missing the #370/#374 zero-load-fallback startup-race guard (#357).** A real HA restart followed by this script running via cron would reproduce the exact confidently-wrong `status: optimal` plan #370/#374 already fixed on the native runtime weeks earlier. Ported `_is_transient_startup_load_forecast_error()` and `compute_forecast_coverage_hours()` (the #112 coverage metric, threaded through both `read_load_forecast_sensor()` and `sum_load_forecasts()`, surfaced on both sensor pushes and the per-cycle status line).

  **Correction, added after release (Mark Purcell, same day):** the original entry above said this was "found live by Mark Purcell's own health-check on his independent install" — not accurate. His install runs the native in-process runtime, not the cron script, so it never actually executed this code path; the bug was real but never live-reproduced on his end. He found it by diffing the integration and standalone copies' own function sets after his earlier 25-function review of #357, not from a live symptom. The fix and its correctness are unaffected — only the provenance claim was wrong.
...
### Fixed
- **#375 (High): LTS/hybrid rows never became training rows** ([#375](https://github.com/code-imstillalive/nimbus/issues/375), thanks @purcell-lab -- filed by his own IV&V agent as a sub-issue of #371, after v0.94.116's #372 fix stopped the `lts`/`hybrid` crash but a real install still trained on only ~470 usable points instead of the full multi-month history). `coordinator.py`'s `lts`/`hybrid` sources feed `train_model()` events derived from HA's hourly long-term-statistics buckets -- a coarser cadence than the 15-min training grid. #350's `resample_observed_mask()` correctly marks only the first grid point after each new hourly event as a genuine observation, but that one observation's own `lag_short` lookup (one grid step, 15 min, earlier) lands on the *previous* hour's forward-filled tail -- on a real wall-clock grid (which essentially never lines up exactly on the hour), that tail point is routinely ~50 minutes stale relative to the event it's filled from. The default staleness cap (`resample_minutes * MAX_TRAINING_STALENESS_GRID_STEPS` = 45 minutes, #353's own outage-detection guard, sized for fine-cadence recorder data) rejected that lookup outright, making the whole row's `lag_short_v` `None` -- silently dropping *every* LTS-derived row, not just the forward-fill duplicates #350 meant to remove. **Correction, added after release**: the devhub log lines originally cited here as an independent live reproduction (`Only 46/32/31/56/276 usable training points`) were re-checked directly against devhub's own hub config after this fix shipped, and are NOT this bug -- devhub's `entry.options` has no `training_source` key set at all, so every one of its subentries trains in the default `recorder`-only mode, never `lts`/`hybrid`, and this fix only changes the staleness cap for those two sources. The real explanation for those specific low counts is mundane: those subentries simply don't have 500 real recorder-cadence points of history yet. A genuinely wrong attribution, caught by checking the live config before trusting a superficially-matching log line -- see CLAUDE.md's own note on this for the fuller writeup. This does not change the fix itself (root-caused and mutation-tested against real synthetic data, independent of any devhub claim) or its correctness for an install that genuinely does run `lts`/`hybrid`.
  Fixed with a new `max_staleness_minutes` parameter on `train_model()` (`ml/model.py` stays source-agnostic -- it has no idea what "LTS" is); `coordinator.py` passes `_LTS_PERIOD_MINUTES + RESAMPLE_MINUTES` (60 + 15 = 75 minutes) whenever `training_source` is `lts`/`hybrid`, wide enough to cover the worst-case ~59-minute gap while still catching a genuine multi-hour+ outage exactly like #353 always did. New dedicated regression test (`tests/test_train_model_lts_staleness_cap.py`) reproduces the live bug exactly (30 days of hourly events on a 15-min grid with a non-zero offset train to `None` under the old 45-min cap) and confirms the fix recovers ~24 rows/day under the new 75-min cap. Mutation-tested: reverting the fix reproduces the identical `Only 0 usable training points` symptom.
  This is the third and last of the three coordinated fixes (#372, #373, #375) for the IV&V-reported day-long forecast outage -- may also directly resolve #374's own empty-forecast fallback, since a subentry that never had usable training data would fall straight into that fallback path; needs live confirmation on devhub, not assumed.
...
### Fixed
- **#365 finding 4 (remaining half): a dispatched executor solve can no longer outlive `async_unload_entry()`** ([#365](https://github.com/code-imstillalive/nimbus/issues/365), thanks @purcell-lab — v0.94.101 already fixed this finding's globals-reset half; this closes the harder half explicitly left open at the time). `hass.async_add_executor_job(_run_one_cycle, hass)` returns a genuine `asyncio.Future` backed by a worker thread; cancelling whichever Task happens to be *awaiting* that future (as `async_unload_entry` already did for the startup-retry task) does not stop the underlying thread once it has actually started running -- `sw.main()` kept executing, including its own `ha_post_state()` calls, regardless, potentially writing into a torn-down entry's namespace.
  `solver_runtime.async_run_solve()` (the single funnel point every solve trigger -- periodic cron, price-watcher debounce, startup-retry -- already goes through) now records its dispatched future in a module-level reference; a new `wait_for_in_flight_solve()`, called from `async_unload_entry()` after every solve trigger is cancelled/unsubbed but *before* platforms are torn down, awaits it (bounded by the same 120s `_SLOW_CYCLE_THRESHOLD_S` already used elsewhere to flag a suspiciously slow cycle, via `asyncio.shield` so a timeout on our own wait never disturbs whichever original caller is also awaiting the same future). This closes the race by construction: by the time platform teardown starts, no executor job can still be running.
  Verified with real `asyncio.Future` objects (matching `hass.async_add_executor_job`'s actual return type) rather than mocking away the underlying mechanism -- an ordering test confirms platform teardown only happens after an in-flight solve resolves, a timeout test confirms unload never blocks indefinitely, and a shield test confirms the original caller still gets its real result even after our own wait times out. Mutation-tested: removing the new `await` from `async_unload_entry()` was confirmed to fail the ordering test.
  **Also fixed, found live while adding this test**: `tests/test_services.py` had two tests (`test_solve_now_calls_async_run_solve`, `test_solve_now_logs_a_warning_on_a_failed_solve_but_does_not_raise`) that replaced `solver_runtime.async_run_solve` with a raw attribute assignment and never restored it -- since `solver_runtime` is a module object shared by every importer, this permanently monkeypatched the REAL function for the rest of the pytest process once either test ran, silently breaking any later test relying on the genuine implementation (this issue's own new test failed only when run after `test_services.py`, never in isolation). Fixed with `unittest.mock.patch.object`, matching the auto-restoring convention this project's other `solver_runtime` call-site tests already use.
